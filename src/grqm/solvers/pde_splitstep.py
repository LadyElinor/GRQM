...
"""Numerical kernels for the GRQM toy Schrödinger-Newton lane.

These solvers are part of the model, not neutral plumbing. In particular:
- the Poisson solve assumes a periodic 1D domain
- the zero mode is gauge-fixed to zero mean in Fourier space
- optional dispersion terms are diagnostic extensions, not established physics
"""
...
def poisson_solve_periodic_1d(rho: np.ndarray, dx: float, kappa: float) -> np.ndarray:
    ...
    Uses an FFT solve with a zero-mean gauge fix (``phi_k[0] = 0``). This is a
    modeling choice tied to the domain assumptions, so downstream interpretation
    should not treat it as invisible implementation detail.
    """
    n = rho.size
...
def split_step_sn_1d(
    ...
    Audit note: accuracy claims should be paired with explicit timestep,
    grid-resolution, and boundary-condition sensitivity checks rather than read
    directly off smooth-looking output.
    """
