...
"""1D Schrödinger-Newton toy model wrapper.

This module is useful as a receipts-bearing diagnostic sandbox, not as a full
relativistic or empirically validated GR/QM unification engine.

Interpretation notes:
- the Poisson solve and time evolution occur on a periodic 1D domain
- units are effectively toy or nondimensionalized unless documented otherwise
- Q1/Q2/Q3 diagnostics are only meaningful when paired with stated regime,
  tolerance, and falsifier notes
"""
...
def run_sn_1d(params: SNParams, seed: int = 0):
    """Run 1D Schrödinger–Newton toy evolution and compute first-pass diagnostics.

    Audit note: these diagnostics are governance-facing probe metrics, not by
    themselves evidence for a physical mechanism. Strong interpretation should
    also check invariants, boundary assumptions, and sensitivity to domain and
